...
int main(int argc, char* argv[])
{
    ...
    // ASK if is there a better way of doing this:
    Jacobi task_a;
    task_a.jacobi_method(A, R, n);


    // write the data to file
    ofstream targetfile_eigenValues;
    targetfile_eigenValues.open("results_eigenValues.txt");
    for (int i = 0; i < n; i++){
    ...
    targetfile_eigenVectors.open("results_eigenVectors.txt");
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
